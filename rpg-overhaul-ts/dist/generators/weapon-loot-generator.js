...
exports.WeaponLootGenerator = void 0;
const loot_generator_1 = require("./loot-generator");
const config_1 = require("../config/loot/config");
class WeaponLootGenerator extends loot_generator_1.LootGenerator {
    /**
     * Generate weapon loot with full RPG properties
     */
    static generateWeaponLoot(context) {
        const result = [];
        const audit = [];
    ...
    static generateWeapon(context) {
        ...
        const weaponStats = this.applyManufacturerTraits(baseWeapon, manufacturer, rarity);
        // Generate item name
        ...
        const audit = {
            ...
            rarity: rarity,
            tier: this.determineLootTier(context.gamestage) === "tier_1"
                ? 1
     ...
     * Determine how many weapons to generate
     */
    static determineWeaponCount(context) {
        const tier = this.determineLootTier(context.gamestage);
        const tierConfig = config_1.LootConfigLoader.loadTierConfig()[tier];
        if (!tierConfig)
            return 0;
     ...
     */
    static selectBaseWeapon(context) {
     ...
     */
    static selectManufacturer(context) {
     ...
     */
    static determineWeaponRarity(gamestage) {
     ...
     */
    static applyManufacturerTraits(baseWeapon, manufacturer, rarity) {
        ...
        return {
            damage: Math.floor(baseStats.damage * modifiers.damage * rarityMultiplier),
     ...
     */
    static getManufacturerModifiers(manufacturer) {
        const modifiers = {
            Atlas: {
    ...
    static exportWeaponAuditToCSV() {
        ...
        const rows = this.weaponAudit.map((entry) => headers.map((header) => entry[header]).join(","));
        return [headers.join(","), ...rows].join("\n");
    }
}
exports.WeaponLootGenerator = WeaponLootGenerator;
WeaponLootGenerator.weaponAudit = [];
//# sourceMappingURL=weapon-loot-generator.js.map
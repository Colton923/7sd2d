...
exports.SimpleXMLGenerator = void 0;
class SimpleXMLGenerator {
        ...
        for (const item of items) {
            ...
            if (item.properties) {
                for (const prop of item.properties) {
                    ...
                    if (prop.param1)
                        propLine += ` param1="${prop.param1}"`;
                    if (prop.param2)
                        propLine += ` param2="${prop.param2}"`;
                    propLine += ' />';
                    lines.push(propLine);
                }
            }
    ...
    generateEntities(entities) {
        ...
        for (const entity of entities) {
            ...
            if (entity.extends)
                entityLine += ` extends="${entity.extends}"`;
            entityLine += '>';
            ...
            if (entity.drops) {
                for (const drop of entity.drops) {
                    ...
                    if (drop.count)
                        dropLine += ` count="${drop.count}"`;
                    if (drop.prob)
                        dropLine += ` prob="${drop.prob}"`;
                    if (drop.stick_chance)
                        dropLine += ` stick_chance="${drop.stick_chance}"`;
                    dropLine += ' />';
                    lines.push(dropLine);
                }
            }
    ...
    generateRecipes(recipes) {
        ...
        for (const recipe of recipes) {
            ...
            if (recipe.craft_area)
                recipeLine += ` craft_area="${recipe.craft_area}"`;
            if (recipe.craft_time)
                recipeLine += ` craft_time="${recipe.craft_time}"`;
            if (recipe.craft_exp_gain)
                recipeLine += ` craft_exp_gain="${recipe.craft_exp_gain}"`;
            if (recipe.tags)
                recipeLine += ` tags="${recipe.tags}"`;
            recipeLine += '>';
    ...
    generateLootContainers(containers) {
        ...
        for (const container of containers) {
            ...
            if (container.count)
                containerLine += ` count="${container.count}"`;
            if (container.size)
                containerLine += ` size="${container.size}"`;
            containerLine += '>';
            ...
            if (container.items) {
                for (const item of container.items) {
                    ...
                    if (item.count)
                        itemLine += ` count="${item.count}"`;
                    if (item.prob)
                        itemLine += ` prob="${item.prob}"`;
                    if (item.quality)
                        itemLine += ` quality="${item.quality}"`;
                    itemLine += ' />';
                    lines.push(itemLine);
                }
            }
            if (container.lootgroups) {
                for (const group of container.lootgroups) {
                    ...
                    if (group.count)
                        groupLine += ` count="${group.count}"`;
                    if (group.prob)
                        groupLine += ` prob="${group.prob}"`;
                    groupLine += ' />';
    ...
    generateBlocks(blocks) {
        ...
        for (const block of blocks) {
            ...
            if (block.extends)
                blockLine += ` extends="${block.extends}"`;
            blockLine += '>';
...
exports.SimpleXMLGenerator = SimpleXMLGenerator;
//# sourceMappingURL=simple-xml-generator.js.map
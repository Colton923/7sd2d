...
// Import base types - avoiding circular dependency
...
export interface WeaponTemplate {
  id: string;
  name: string;
  baseType: string;  // Game's base item ID
  weaponClass: WeaponClass;
  description: string;
  tier: number;  // 1-6
  statModifiers?: Partial<WeaponBaseStats>;  // Overrides to class stats
  specialProperties?: {
    burstFire?: number;
    chargeTime?: number;
    overheat?: boolean;
  ...
  requiredParts?: string[];  // Specific parts required
  bannedParts?: string[];    // Parts that can't be used
  fixedAffixes?: string[];   // Always has these affixes
}
...
export function getWeaponTemplate(id: string): WeaponTemplate | undefined {
  return weaponTemplates.find(t => t.id === id);
}

// Get templates by class
export function getTemplatesByClass(weaponClass: WeaponClass): WeaponTemplate[] {
  return weaponTemplates.filter(t => t.weaponClass === weaponClass);
}

// Get templates by tier
export function getTemplatesByTier(tier: number): WeaponTemplate[] {
  return weaponTemplates.filter(t => t.tier === tier);
}

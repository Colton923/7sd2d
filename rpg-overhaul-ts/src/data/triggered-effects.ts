...
export const triggeredEffectTemplates: Record<string, TriggeredEffectTemplate> = {
  // ========== OFFENSIVE EFFECTS ==========
  heal: {
    effect: 'heal',
    name: 'Life Steal',
...
export function generateRandomTriggeredEffects(
  level: number,
  rarity: string,
  weaponType: string
): TriggeredEffect[] {
  ...
  let numEffects = 0;
  switch (rarity) {
    case 'common': numEffects = Math.random() < 0.1 ? 1 : 0; break;
    case 'uncommon': numEffects = Math.random() < 0.3 ? 1 : 0; break;
    case 'rare': numEffects = Math.random() < 0.6 ? 1 : Math.random() < 0.2 ? 2 : 0; break;
    case 'epic': numEffects = Math.random() < 0.8 ? 2 : 1; break;
    case 'legendary': numEffects = Math.random() < 0.5 ? 3 : 2; break;
    default: numEffects = 0;
  }
  
  const availableEffects = Object.values(triggeredEffectTemplates);
  const selectedEffects = new Set<string>();
  
  for (let i = 0; i < numEffects && availableEffects.length > 0; i++) {
    // Pick a random effect that hasn't been selected
    let effect: TriggeredEffectTemplate;
    do {
...
export function getCompatibleTriggers(weaponType: string): TriggerType[] {
  ...
  allTriggers.push(
    'onDodge', 'onSprint', 'onCrouch', 'onJump', 
    'onSwapWeapon', 'onNightfall', 'onDaybreak', 'onBloodMoon'
  );

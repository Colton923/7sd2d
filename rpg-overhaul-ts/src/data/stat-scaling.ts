...
export interface StatScaleRange {
  minScale: number;  // Minimum scaling factor (e.g., 0.5 = 50% of base)
  maxScale: number;  // Maximum scaling factor (e.g., 1.5 = 150% of base)
}
 ...
 */
export const tierScalingRanges: Record<number, StatScaleRange> = {
 ...
 */
export function scaleWeaponStats(
  baseStats: WeaponBaseStats,
  playerLevel: number,
  weaponTier: number,
  quality: number = 300
): WeaponBaseStats {
  const tierRange = tierScalingRanges[weaponTier] || tierScalingRanges[1];
  ...
  const scaledStats: WeaponBaseStats = {
    // Damage scales fully
 ...
 */
export function getItemLevelSuffix(itemLevel: number): string {
  if (itemLevel < 10) return 'Crude';
  if (itemLevel < 20) return 'Worn';
  if (itemLevel < 30) return 'Standard';
  if (itemLevel < 40) return 'Quality';
  if (itemLevel < 50) return 'Superior';
  if (itemLevel < 60) return 'Exceptional';
  if (itemLevel < 70) return 'Elite';
  if (itemLevel < 80) return 'Masterwork';
  if (itemLevel < 90) return 'Legendary';
  return 'Godlike';
}
...
export function applyStatVariance(
  ...
  const randomVariance = (stat: number) => {

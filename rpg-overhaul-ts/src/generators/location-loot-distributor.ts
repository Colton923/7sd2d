...
export class LocationLootDistributor {
  /**
   * Generate location-specific loot with biome considerations
   */
  static generateLocationLoot(
  ...
  private static generateBaseLocationLoot(
    ...
    const items: GeneratedLootItem[] = [];
    const spawnLocations = LootConfigLoader.getSpawnLocations();
  ...
  private static generateBiomeSpecialLoot(
    ...
    if (!context.biome) return items;

    const biomeDrops = LootConfigLoader.getSpecialDropsByBiome();
    const biomeConfig = biomeDrops[context.biome];

    if (!biomeConfig) return items;

    // Generate special drops for this biome
    const specialItems = this.selectBiomeSpecialDrops(
      biomeConfig,
      context.difficulty
    );
  ...
  private static selectBiomeSpecialDrops(
    biomeConfig: SpecialDropByBiome,
    difficulty: number
  ): string[] {
  ...
  private static applyLocationModifiers(
    ...
    if (!locationConfig) return items;
   ...
   */
  private static getContainerLootTable(containerType: string): Array<{
    itemName: string;
    probability: number;
    ...
    const lootTables: { [container: string]: any[] } = {
      medicine_cabinet: [
   ...
   */
  private static determineBiomeRarity(difficulty: number): string {
  ...
  private static calculateItemValue(
    ...
    const baseValues: { [item: string]: number } = {
      bandage: 5,
      firstAidKit: 25,
      painkillers: 8,
      cannedFood: 12,
   ...
   */
  private static calculateSpecialItemValue(itemName: string): number {
    const specialValues: { [item: string]: number } = {
      witchsBrew: 75,
      cursedBone: 50,
      swampGem: 100,
      oasisWater: 60,
   ...
   */
  static getLocationLootProbability(
    location: string,
    gamestage: number
  ): { baseProbability: number; lootAbundance: number } {
    const spawnLocations = LootConfigLoader.getSpawnLocations();

...
export class LootGenerator {
  ...
  static generateLoot(context: LootGenerationContext): LootGenerationResult {
    const result: LootGenerationResult = {
      items: [],
      totalValue: 0,
      rarityDistribution: {},
      categoryBreakdown: {},
    ...
    const tierConfig = LootConfigLoader.loadTierConfig()[lootTier];
    if (!tierConfig) {
  ...
  private static generateSingleItem(
  ...
  ): GeneratedLootItem | null {
    if (availableCategories.length === 0) return null;

    // Select random category
    const category =
      availableCategories[
        Math.floor(this.random() * availableCategories.length)
      ];

    // Get category config
    const categoryConfig = LootConfigLoader.getLootCategories()[category];
    ...
    const itemProperties = this.generateItemProperties(
      category,
      rarity,
      context
    );
  ...
  protected static determineLootTier(gamestage: number): string {
    if (gamestage <= 25) return "tier_1";
    if (gamestage <= 100) return "tier_2";
    if (gamestage <= 300) return "tier_3";
    if (gamestage <= 1000) return "tier_4";
    return "tier_5";
  }
  ...
  private static getAvailableCategories(
    ...
    for (const [categoryName, category] of Object.entries(allCategories)) {
      ...
      if (
        category.spawn_locations.length > 0 &&
   ...
   */
  private static determineRarity(gamestage: number, category: string): string {
   ...
   */
  private static rollItemCount(tierConfig: any): number {
   ...
   */
  private static rollQuantity(category: string): number {
    const scaling = LootConfigLoader.getLootQuantityScaling();
    const tier = this.determineLootTier(100); // Default tier for quantity
    const tierScaling = scaling[tier];

    if (tierScaling) {
      return (
        Math.floor(this.random() * (tierScaling.max - tierScaling.min + 1)) +
        tierScaling.min
      );
    }
  ...
  private static calculateItemValue(
    ...
    const economicScaling = LootConfigLoader.getEconomicScaling();
